...
const ImagesManagement: React.FC = () => {
  ...
  const [selectedFiles, setSelectedFiles] = useState<File[]>([]); // Allow multiple files
  ...
  const handleAddImages = async () => {
    ...
    try {
      // Upload each image
      await Promise.all(
        selectedFiles.map(file => uploadImage({ product: selectedProductId, image: file }))
      );
      setSelectedFiles([]);
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-gray-800 p-6 rounded shadow-lg">
            ...
            <div className="flex justify-end">
              <button onClick={handleAddImages} className="bg-gray-600 text-white p-2 rounded mr-2">
                Add
              </button>
